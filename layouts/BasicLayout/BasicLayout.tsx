...
//Query de productos
const fetcher = () => fetch('https://app-node-nextjs.herokuapp.com/api/productosActivos').then((res) => res.json())
                ...
                {data?.productos?.map((producto: any) => (
                    <div key={producto._id} className="md:bg-white shadow-lg rounded-lg  md:flex my-4 mx-2">
                        ...
                        <div className="bg-white shadow-lg px-6 pb-6">
                            ...
                            <div className="add-btn" onClick={() => { mutation.mutate({ usuario: usuario.value._id, producto: producto._id, cantidad: "1" }) }}><h1 className="symbol mr-2 text-white">+</h1><button className="text-white">Agregar</button></div>
                        </div>
                    </div>
                ))}
            </div>
